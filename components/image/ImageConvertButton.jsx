...
export default function ImageConvertButton() {
  const { file, loading, convert, loaded, format } = useImage()

  const isEngineReady = loaded
  const isFileSelected = !!file
  const isConverting = loading

  let label = "Convert Image"
  let disabled = true
  ...
  return (
    <button
      ...
      className={`w-full px-6 py-4 rounded-xl font-bold text-sm uppercase tracking-wider transition-all duration-200 flex items-center justify-center gap-2
        ${!disabled
          ? "bg-blue-500 text-white hover:bg-blue-400 hover:scale-[1.02] hover:shadow-lg hover:shadow-blue-500/20"
